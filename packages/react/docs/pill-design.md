...
- **A long host label truncates with an ellipsis** (this.i `k7pq2mzv`), or with a plain cut when the host passes `textOverflow="clip"`. When it is measured as truncated, hover or keyboard focus scrolls it end to end and back (a Web Animations API animation of `transform` on an inner text span, with literal pixel keyframes; the span is inline at rest so the ellipsis still works, and inline-block only while it moves), and it returns when the pointer or focus leaves. A label that fits never moves. The full label also joins the tooltip, above the value preview. Under `prefers-reduced-motion` nothing moves: the tooltip carries the label for pointer users, and focus wraps it in place for keyboard users. The accessible name always holds the full label. None of this reopens §14: a label is first-party host text, not value-derived.
- **The mnemonic is fitted, never cut** (§13). It is value-derived, and a truncated mnemonic would show only its first cell, which is the short grindable teaser §3.3 forbids. So it never ellipsizes and never scrolls. It shows at full size if it fits, else at 85% font size, else it is dropped and the type text takes the slot.
  ...
  *Changed 2026-09-25 (v0.18.2):* the scroll now animates `transform` on an inner span instead of the label's `text-indent`. In Brave 147 (Chromium) a `text-indent` animation moved the text in layout but was not repainted: a one-pixel shimmer, then a single jump when something else forced a repaint. Headless Chromium 149 painted it, so the e2e suite could not see the fault; a diagnostic log from Brave showed layout moving smoothly while the screen did not. A transform is composited, so every frame is drawn. The label also sets `text-align: start`, since its `<button>` ancestor centres text. The `textOverflow` prop (`ellipsis` | `clip`) was added in the same release.
