...
- **A long host label truncates with an ellipsis** (this.i `k7pq2mzv`), or with a plain cut when the host passes `textOverflow="clip"`. When it is measured as truncated, hover or keyboard focus scrolls it end to end and back (a Web Animations API animation of `text-indent` with literal pixel keyframes), and it returns when the pointer or focus leaves. A label that fits never moves. The full label also joins the tooltip, above the value preview. Under `prefers-reduced-motion` nothing moves: the tooltip carries the label for pointer users, and focus wraps it in place for keyboard users. The accessible name always holds the full label. None of this reopens §14: a label is first-party host text, not value-derived.
- **The mnemonic is fitted, never cut** (§13). It is value-derived, and a truncated mnemonic would show only its first cell, which is the short grindable teaser §3.3 forbids. So it never ellipsizes and never scrolls. It shows at full size if it fits, else at 85% font size, else it is dropped and the type text takes the slot.
  ...
  *Changed 2026-09-25 (v0.18.2):* the scroll moved from CSS `@keyframes` to the Web Animations API. The keyframes' end value was `calc(-1 * var(--overflow))`, and Brave animated it discretely: nothing for half the sweep, then one jump to the end. Literal pixel keyframes leave an engine nothing to fall back on. The `textOverflow` prop (`ellipsis` | `clip`) was added in the same release.
