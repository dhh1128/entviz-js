...
export interface CompareMessages {
  ...
  unknownRasterSimilarText: string; // raster look-alike AND a crude, no-OCR text-pattern check agreed
...
export const defaultCompareMessages: CompareMessages = {
  ...
  unknownRasterSimilarText: "No visible difference in the colors or the crude text pattern we can sample — but an image can’t be read for text, so this isn’t a proven match. Walk the cells to check.",
