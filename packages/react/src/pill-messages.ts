...
const CATALOG: Record<string, Messages> = {
  ...
  "zh-Hans": { view: "查看可视化", ariaView: "查看可视化，{type}", truncated: "指纹", actions: "操作", copyValue: "复制值", copyComparison: "复制比较文本", copyImage: "复制图片", copySvg: "复制 SVG", copiedValue: "已复制值 · {n} {unit}", copiedComparison: "已复制比较文本 · {n} 个单元格", copiedImage: "已复制图片", copiedSvg: "已复制 SVG", copyFailed: "复制失败", desc: "可视化通道 — 类型 {type}；单元格 {cells}；颜色条 {bars}；四分位单元格 {quartiles}；空白图 {bmin} 至 {bmax}；色条标记 {bleft} 和 {bright}，共 {bslots}。", stepCite: "引用", stepVisualize: "可视化", stepCompare: "比较", teachVisualize: "这是完整的可视化图 — 请逐格阅读以核对数值；仅凭一瞥并不可靠。", compareAction: "与参照进行比较…" },
