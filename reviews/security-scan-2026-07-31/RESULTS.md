...
> **FIXED** (2026-08-06), both halves. The cap now runs where it is reachable: at the top of `characterize()` (`packages/core/src/characterize.ts`) and at the top of `classifyInput` **before** `parse()`, covering both arms rather than only the UTF-8 fallback (`packages/core/src/entviz.ts`). That is every public route to the decode — `describeChannels`, `comparisonText`, `gridShapes`, `classifyValue`, and `compare.ts`'s `identityKey` all reach it through `classifyInput`; the React pill reaches it through `characterize`. `render()` keeps its own check. And the decode itself is no longer quadratic: `decodedBytesInteger` collects digits and hands them to `digitsToInt`, a balanced divide-and-conquer fold (split in half, combine as `hi * base**len(lo) + lo`) with memoized combine exponents — ~O(n^1.58). **Measured** at the 64 KiB cap on node 24: the naive fold 740 ms, the balanced fold 17.5 ms (25.4 ms without the exponent memo), `characterize()` end-to-end ~30 ms. Locked in `packages/core/test/unit/decode-speed-and-exactness.test.ts` (35 assertions), a port of the reference's `tests/test_decode_speed_and_exactness.py`.
>
> Note on the fix as suggested: the `ceil(core.length * log2(base) / 8)` size estimate was **evaluated and rejected**, not overlooked. `sizeBits` is spec-normative (`docs/spec.md` *Resolution A*) and feeds the SIZE label, and the estimate disagrees with the exact value whenever leading digits are zero — a Bitcoin P2PKH address measures 192 bits exactly and 200 by the estimate; a base58 core of all `1`s measures 8 and estimates 160. Substituting it would change rendered output and break conformance. The exact integer is kept and computed faster instead; conformance stayed 104/104 with no golden moved. A test asserts explicitly that the estimate is **not** equivalent, so nobody swaps it in later. Rationale at `this.i:pzxcxqtd`; the reference's parallel fix is `entviz` commit 77d35cc (`this.i:f4std3c0`).
...
> **FIXED** (2026-08-06), by the same change as F3. `characterize()` enforces `MAX_INPUT_CHARS` itself and throws `inputTooLargeMessage(...)` before `parse()` or any decode runs, so the pill's `characterize(value.trim())` at `EntvizPill.ts:307` is now bounded by the same cap that bounds `render()` — and the pill's existing `try/catch` around that memo surfaces it through `onError` as an ordinary over-cap rejection, with no new failure mode. The guard was **not** placed in `EntvizPill`: the sibling-runs-first defect is general, so it is fixed at the core entry point rather than at one caller. `packages/core/test/unit/decode-speed-and-exactness.test.ts` pins the throw at `MAX_INPUT_CHARS + 1`, the trim-then-cap ordering, and completion at the cap for the finding's own `z1`-repeated base58 payload.
...
> **FIXED** (2026-08-06). `onFetch` (`packages/react/src/EntvizCompare.ts`) re-derives the origin from the response — `originOf(res.url)` — and uses that everywhere the pre-redirect origin used to appear. `fetch.success` now carries the origin the bytes came from, plus `requestedOrigin` when a redirect moved them (`packages/react/src/events.ts`), so a host log records the discrepancy instead of the fiction. Because provenance here is judgment-bearing locked copy, re-deriving alone was judged insufficient: a **cross-origin** redirect holds the bytes out of the reference entirely and asks again, naming both origins ("trusted.example redirected this to evil.example… use it anyway?"). Accepting installs the reference labelled with the origin that actually served it; discarding, editing the field, or dropping something else throws it away. The three new strings are `VERDICT_LOCKED_KEYS`. A same-origin redirect is adopted silently. Eight tests in `packages/react/test/EntvizCompare.test.tsx` ("a redirected fetch cannot borrow the approved origin") drive a mocked fetch whose `res.url` differs from the requested URL.
>
> Note on the fix as suggested: `redirect: "manual"` was **rejected** — in a browser it yields an opaque-redirect response with an empty `url` and no readable `Location`, so the one fact re-consent needs (the new origin) is exactly what it hides; `redirect: "error"` fails closed but breaks benign http→https and trailing-slash redirects and still cannot name a target. Following and then re-deriving is what makes the true origin knowable; the adoption gate is what makes it consented. Residual limits, stated plainly: the **egress** to the redirect target still happens and cannot be un-sent — only the adoption of the bytes is gated. A host-injected `fetchReference` is unchanged (it is handed the origin up front and returns bytes, with no response URL to re-derive from), so a host that follows redirects inside its own fetcher owns that policy. And `originOf(res.url) || refOrigin` falls back to the requested origin when a fetch stand-in supplies no response URL — real browser responses always do. Rationale at `this.i:kwt1faw7`.
